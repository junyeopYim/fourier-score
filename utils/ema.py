...
class EMA:
    ...
    def update(self,model):
        ...
        for n,p in model.named_parameters():
            if n in self.shadow: self.shadow[n].lerp_(p.detach(),1-decay)

    def state_dict(self):
        return {'decay':self.decay,'num_updates':self.num_updates,'shadow':self.shadow}

    def load_state_dict(self,state,model):
        if state['decay']!=self.decay or set(state['shadow'])!=set(self.shadow): raise ValueError('EMA mismatch')
    ...
    def average_parameters(self,model):
        with torch.no_grad():
            ...
            for n,t in self.shadow.items(): params[n].copy_(t)
        try: yield
        finally:
            with torch.no_grad():
                for n,t in backup.items(): params[n].copy_(t)
