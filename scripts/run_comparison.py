...
from parse_config import load_config,apply_overrides,validate,experiment_name
from model.objectives import OBJECTIVES,COMPARISON_OBJECTIVES


def comparison_runs(config,changes=(),objectives=COMPARISON_OBJECTIVES,seeds=None):
    cfg=load_config(config,changes)
    if not objectives or len(set(objectives))!=len(objectives) or any(x not in OBJECTIVES for x in objectives):
    ...
    for seed in seeds:
        for objective in objectives:
            ...
            if cfg['name']!='auto': arm_changes+=['name='+cfg['name']+f'_{objective}_s{seed}']
            run_cfg=validate(apply_overrides(cfg,arm_changes))
            output=(Path(run_cfg['trainer']['save_dir'])/experiment_name(run_cfg)).resolve()
            if output in outputs: raise ValueError(f'Comparison output collision: {output}')
            outputs.add(output)
            command=[sys.executable,str(Path(__file__).resolve().parents[1]/'train.py'),'-c',str(config)]
            for option in [*changes,*arm_changes]: command+=['--set',option]
...
def check_outputs(runs):
    ...
    for run in runs:
        ...
        if path.exists() and (not path.is_dir() or any(path.iterdir())):
            raise FileExistsError(f'Run exists: {path}; use a new name or resume it with train.py')
...
def main():
    ...
    p.add_argument('--objectives',nargs='+',choices=OBJECTIVES,default=COMPARISON_OBJECTIVES)
    p.add_argument('--seeds',nargs='+',type=int,help='Paired training seeds; defaults to the config seed')
    ...
    if not a.dry_run: check_outputs(runs)
    for run in runs:
        print(json.dumps(run['command']),flush=True)
        if not a.dry_run: subprocess.run(run['command'],check=True)
