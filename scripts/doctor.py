...
def run(device="auto", spectral="auto"):
    cfg = load_config(
        "configs/smoke.json",
        [f"device={device}", f"backend.spectral_transform={spectral}"],
    )
    dev = configure_runtime(cfg)
    result = environment(dev, cfg)
    result["tests"] = []
    for lossname in OBJECTIVES:
        ...
        eps = torch.randn(2, 1, 8, 8, generator=gen).to(dev)
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
        value = training_loss(model, x, lev, eps)
        value.backward()
        norm = torch.nn.utils.clip_grad_norm_(
            model.parameters(), 1.0, error_if_nonfinite=True, foreach=False
        )
        if norm <= 0:
            raise AssertionError("Gradient did not reach backbone")
        optimizer.step()
        model.eval()
    ...
    error = float((got.detach().cpu() - expected).abs().max())
    got.square().sum().backward()
    if error > 2e-4 or not torch.isfinite(x.grad).all():
        raise AssertionError("Spectral device parity failed")
    state = capture_rng(dev)
    a = torch.randn(4, device=dev)
    restore_rng(state, dev)
    b = torch.randn(4, device=dev)
    if not torch.equal(a, b):
        raise AssertionError("Device RNG restore failed")
...
def main():
    ...
    p.add_argument(
        "--spectral", default="auto", choices=["auto", "fft", "matmul", "cpu"]
    )
    p.add_argument("-o", "--output")
    a = p.parse_args()
    result = run(a.device, a.spectral)
    if a.output:
