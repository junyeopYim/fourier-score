...
def main():
    ...
    for objective in OBJECTIVES:
        c = copy.deepcopy(cfg)
        c["loss"]["type"] = objective
        seed_all(c["seed"])
        model = build_model(c, stats, "cpu")
        report = architecture_report(model.backbone)
        result[objective] = {
            k: report[k]
            for k in ("trainable_parameters", "all_parameters", "architecture_sha256")
        }
        result[objective]["initial_backbone_sha256"] = weight_hash(model.backbone)
        result[objective]["attention_blocks"] = sum(
            v == "AttnBlockpp" for v in report["modules"].values()
        )
        result[objective]["biggan_resblocks"] = sum(
            v == "ResnetBlockBigGANpp" for v in report["modules"].values()
        )
        result[objective]["ddpm_resblocks"] = sum(
            v == "ResnetBlockDDPMpp" for v in report["modules"].values()
        )
        del model
    result["identical"] = (
        len({json.dumps(v, sort_keys=True) for v in result.values()}) == 1
    )
    if not result["identical"]:
        raise AssertionError("Architecture or initialization differs across objectives")
    if args.output:
