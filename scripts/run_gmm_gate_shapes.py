"""Fixed paired GMM comparison of sigma-linear and sigma-tanh gates.
...
from fourier_score.gmm import (
    ...
    plateau_arm, shaped_gate_arm, spectral_cap_arm,
)
...
def plot_diagnostics(report, cfg, arms, transition):
    ...
    for arm, label, color in zip(selected, labels, colors):
        ...
        axes[2].fill_between(xs, means-sd, means+sd, color=color, alpha=.1)
    for ax in axes:
        ax.grid(alpha=.18)
        ax.set_xlabel("Noise sigma")
    axes[0].set(xscale="log", ylabel="Gate g", title="Fixed gate shapes")
    ...
    for ax in axes[:2]:
        ax.set_xticks([.1, .3, 1., 1.5, 3.], labels=["0.1", "0.3", "1", "1.5", "3"])
        ax.minorticks_off()
...
def main(argv=None):
    ...
    parser.add_argument("--test-bank-version", default="gmm-linear-tanh-v1")
    ...
    if not args.test_bank_version or args.test_bank_version in (
            "gmm-oracle-v1", "gmm-gated-v1", "gmm-plateau-v1", "gmm-spectral-cap-v1"):
    ...
    if not cfg.sigma_min < args.sigma_switch < cfg.sigma_max:
        parser.error("Gate center must lie within the training sigma range")
    ...
    new_arms = tuple(shaped_gate_arm(cov, mode, args.sigma_switch, args.sharpness)
    ...
    transition_sigmas = sorted({s for s in [*np.linspace(.5, 2.5, 17).tolist(), .8, 1.,
                                           args.sigma_switch,
    ...
    selection = dict(criterion="Fixed before training; no search or test selection", **fixed,
    ...
    if plan.exists():
        ...
        if json.loads(json.dumps(protocol)) != previous:
            raise ValueError("Existing gate-shape protocol differs; choose a new output directory")
    ...
    tasks = [(cfg, lam, seed, arm, output, reuse_directory(roots, lam, seed, arm), provenance)
    ...
    payload = export_report(report, cfg, provenance, results, tested, selection, pairing, arms,
                            figure_stem="gmm_gate_shape_comparison", paired_arms=new_arms)
    ...
    payload.update(n_training_runs=sum(r["origin"] == "trained" for r in results),
                   ...
                   gate_definitions=protocol["gate_definitions"],
