...
class NoiseLevel:
    ...
    def slice(self,start,end):
        return NoiseLevel(self.alpha[start:end],self.sigma[start:end],self.coordinate[start:end])

class NoiseProcess:
    def __init__(self,config):
        self.config=dict(config); self.kind=config['type']
        self.N=config['num_scales']; self.sigma_min=config['sigma_min']; self.sigma_max=config['sigma_max']
        self.t_min=config['t_min']; self.log_ratio=math.log(self.sigma_max/self.sigma_min)
        self.betas=torch.linspace(config['beta_start'],config['beta_end'],self.N,dtype=torch.float64).float()
    ...
    def sample(self,batch_size,device,generator):
        ...
        if self.kind=='ve':
            c=torch.rand(batch_size,generator=generator)*(1-self.t_min)+self.t_min
        else:
            c=torch.randint(self.N,(batch_size,),generator=generator)
        return self.level(c,device)

    def condition(self,level,embedding):
        if embedding=='fourier': return level.sigma
        if self.kind=='ddpm': return level.coordinate.float()
    ...
    def perturb(self,x,level,noise):
        return level.alpha[:,None,None,None]*x+level.sigma[:,None,None,None]*noise

    def bin_coordinate(self,level):
        return level.coordinate.float()/(self.N-1) if self.kind=='ddpm' else level.coordinate
